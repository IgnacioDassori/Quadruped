...
from agents.laikago_house_obstacles import LaikagoHouse as Laikago

class houseEnv(gym.Env):
    '''
    HOUSE ENVIRONMENT WITH OBSTACLES, FIXED FREQ AND NO CORONAL MOTORS
    '''
    def __init__(self, mode, freq_range, gamma, vae_path, spawn_objects=True, env_id=None, log_dir=None):
        super(houseEnv, self).__init__()
        # CPG parameters (Ah, Ak_st, Ak_sw, d) (x4)
        ...
        # NEW: add robot position and goal position to observation space (x3) goal Y-axis is fixed
        # roll, pitch, yaw (x3), angular velocities (x3), motor positions (x8), phase & CPG parameters (x5)
        # latent vector (X32)
    ...
    def reset(self):
        ...
        self.quadruped = Laikago(client=self.client, gamma=self.gamma, obstacles_pos=sm.obstacles_pos)
        ...
        # Initial motor positions given by CPG
        initial_motor_positions = self.quadruped.CPG.get_angles()
