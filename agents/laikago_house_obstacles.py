...
from utils.cpg import CPG_houseV2 as CPG

class LaikagoHouse:
    '''
    AGENT FOR HOUSE ENVIRONMENT WITH OBSTACLES
    '''
    def __init__(self, client, dt=1./500, gamma=5.0, obstacles_pos=[]):
        self.client = client
        ...
        self.lower_obs = np.array([-5.0, -5.0, -5.0,
                                   -math.pi/2, -math.pi/2, 0, -10.0, -10.0, -10.0,
                                   -0.6, -1.6, -0.6, -1.6, -0.6, -1.6, -0.6, -1.6,
                                   0, 0, 0, 0, 0.5,
                                   -250, -250, -250, -250, -250, -250, -250, -250, -250, -250, -250, -250, -250, -250, -250, -250,
                                   -250, -250, -250, -250, -250, -250, -250, -250, -250, -250, -250, -250, -250, -250, -250, -250
        ])
        self.upper_obs = np.array([5.0, 5.0, 5.0,
                                   math.pi/2, math.pi/2, math.pi, 10.0, 10.0, 10.0,
    ...
    def apply_action(self, action):
        ...
        for i, motor_id in enumerate([0, 4, 8, 12]):
            p.setJointMotorControl2(
                self.laikago,
                motor_id,
                p.POSITION_CONTROL,
    ...
    def get_observation(self, timestep, encoder):
        ...
        poses = [self.pos[0], self.pos[1], self.goal[0]]
        ...
        observation = np.array(poses+self.ori+list(self.vel[1])+self.motor_positions+cpg_params)
    ...
    def calculate_reward(self, done, timestep):
        ...
        obs_distances = [self.euclidean_dist(self.pos[:2], obs) for obs in self.obstacles_pos]
        r6 = sum([1-x if x < 1 else 0 for x in obs_distances])

        # nullify rewards when falling
        #falling = (abs(a_vel[0])>2.0 or abs(a_vel[1])>2.0)   

        # term weight depends on w
        #R = (1-w)*r1 + w*r2 + r3 -0.3*r4 - r5 - 3*r6 + r7
        R = max(1-2*w,0)*r1 + min(2*w,1)*r2 + r3 - 0.3*r4 - r5 - 5*r6
        #print(f"R: {R}, r1: {r1}, r2: {r2}, r3: {r3}, r4: {r4}, r5: {r5}, r6: {r6}, r7: {r7}")
    ...
    def is_done(self, timestep):
        ...
        if self.angle_to_goal > math.pi/2:
            return True
        # robot falls backwards
        if self.pos[1]<-4.6:
            return True
        # collision with obstacles
        for x in p.getContactPoints(self.laikago):
            if x[2] in [i for i in range(2, self.laikago)]:
                return True
