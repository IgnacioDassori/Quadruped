...
class CPG_house:
    def __init__(self, dt, gamma=5.0):
        ...
        # CORONAL PLANE MOTOR ANGLES
        self._coronal_angles = [0.0, 0.0, 0.0, 0.0]

    def update(self, updates):
        self._phases = (self._phases + 2*math.pi*self._f*self._dt) % (2*math.pi)
        # change parameters gradualy
        #self._f += (updates[0]-self._f)*self._gamma*self._dt
        self._Ah += (updates[0]-self._Ah)*self._gamma*self._dt
        self._Ak_st += (updates[1]-self._Ak_st)*self._gamma*self._dt
        self._Ak_sw += (updates[2]-self._Ak_sw)*self._gamma*self._dt
        self._d += (updates[3]-self._d)*self._gamma*self._dt
        # change coronal motor angles
        self._coronal_angles = updates[4:]

    def get_angles(self):
        # maybe change to make all calculations on an array
        motor_angles = []
        for phi, i in zip(self._phases, range(len(self._phases))):
            motor_angles.append(self._coronal_angles[i])
            # hip phase
            if phi < 2*math.pi*self._d:
                phi_h = phi/(2*self._d)
            else:
            ...
            '''
            hip_angle += self._off_h    
            motor_angles.append(hip_angle*-1)
            # knee amplitude
            phi_k = (phi_h) % (2*math.pi)
            if phi_k < math.pi:
                Ak = self._Ak_st
            ...
            # knee angle
            knee_angle = Ak*g + self._off_k
            motor_angles.append(knee_angle*-1)
